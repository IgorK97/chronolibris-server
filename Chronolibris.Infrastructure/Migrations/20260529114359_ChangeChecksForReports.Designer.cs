// <auto-generated />
using System;
using Chronolibris.Domain.Entities;
using Chronolibris.Infrastructure.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace Chronolibris.Infrastructure.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260529114359_ChangeChecksForReports")]
    partial class ChangeChecksForReports
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.22")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "content_nature_enum", new[] { "unknown", "document", "work", "analysis" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "person_role_kind", new[] { "content", "book", "both" });
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("BookSelection", b =>
                {
                    b.Property<long>("book_id")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<long>("selection_id")
                        .HasColumnType("bigint")
                        .HasColumnName("selection_id");

                    b.HasKey("book_id", "selection_id")
                        .HasName("pk_book_selection");

                    b.HasIndex("selection_id")
                        .HasDatabaseName("ix_book_selection_selection_id");

                    b.ToTable("book_selection", (string)null);
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Book", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Bbk")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("bbk");

                    b.Property<long>("CountryId")
                        .HasColumnType("bigint")
                        .HasColumnName("country_id");

                    b.Property<string>("CoverPath")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("cover_path");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("description");

                    b.Property<bool>("HasHistoricalVersions")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("has_historical_versions");

                    b.Property<string>("ISBN")
                        .HasMaxLength(17)
                        .HasColumnType("character varying(17)")
                        .HasColumnName("isbn");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("boolean")
                        .HasColumnName("is_available");

                    b.Property<bool>("IsReviewable")
                        .HasColumnType("boolean")
                        .HasColumnName("is_reviewable");

                    b.Property<long>("LanguageId")
                        .HasColumnType("bigint")
                        .HasColumnName("language_id");

                    b.Property<long?>("PublisherId")
                        .HasColumnType("bigint")
                        .HasColumnName("publisher_id");

                    b.Property<string>("Source")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("source");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("title");

                    b.Property<string>("Udk")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("udk");

                    b.Property<DateTime?>("UpdatedAt")
                        .IsConcurrencyToken()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<int?>("Year")
                        .HasColumnType("integer")
                        .HasColumnName("year");

                    b.HasKey("Id")
                        .HasName("pk_books");

                    b.HasIndex("CountryId")
                        .HasDatabaseName("ix_books_country_id");

                    b.HasIndex("CreatedBy")
                        .HasDatabaseName("ix_books_created_by");

                    b.HasIndex("LanguageId")
                        .HasDatabaseName("ix_books_language_id");

                    b.HasIndex("PublisherId")
                        .HasDatabaseName("ix_books_publisher_id");

                    b.ToTable("books", null, t =>
                        {
                            t.HasCheckConstraint("CK_books_description_min_length", "LENGTH(TRIM(description)) >= 120");

                            t.HasCheckConstraint("CK_books_title_alnum", "title ~ '[[:alnum:]]'");

                            t.HasCheckConstraint("CK_books_year_range", "year IS NULL OR (year>=-10000 AND year <= EXTRACT(YEAR FROM CURRENT_DATE)+1)");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookContent", b =>
                {
                    b.Property<long>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<long>("ContentId")
                        .HasColumnType("bigint")
                        .HasColumnName("content_id");

                    b.HasKey("BookId", "ContentId")
                        .HasName("pk_book_content");

                    b.HasIndex("ContentId")
                        .HasDatabaseName("ix_book_content_content_id");

                    b.ToTable("book_content", (string)null);

                    b.HasData(
                        new
                        {
                            BookId = 1L,
                            ContentId = 1L
                        },
                        new
                        {
                            BookId = 2L,
                            ContentId = 2L
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFile", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("completed_at");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<long>("FormatId")
                        .HasColumnType("bigint")
                        .HasColumnName("format_id");

                    b.Property<DateTime?>("HiddenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("hidden_at");

                    b.Property<bool?>("HistoricalText")
                        .HasColumnType("boolean")
                        .HasColumnName("historical_text");

                    b.Property<bool>("IsReadable")
                        .HasColumnType("boolean")
                        .HasColumnName("is_readable");

                    b.Property<long>("OriginalSize")
                        .HasColumnType("bigint")
                        .HasColumnName("original_size");

                    b.Property<long>("StatusId")
                        .IsConcurrencyToken()
                        .HasColumnType("bigint")
                        .HasColumnName("status_id");

                    b.Property<string>("StorageUrl")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("storage_url");

                    b.Property<long>("StoredSize")
                        .HasColumnType("bigint")
                        .HasColumnName("stored_size");

                    b.HasKey("Id")
                        .HasName("pk_book_files");

                    b.HasIndex("CreatedBy")
                        .HasDatabaseName("ix_book_files_created_by");

                    b.HasIndex("FormatId")
                        .HasDatabaseName("ix_book_files_format_id");

                    b.HasIndex("StatusId")
                        .HasDatabaseName("ix_book_files_status_id");

                    b.HasIndex("BookId", "FormatId", "HistoricalText")
                        .IsUnique()
                        .HasDatabaseName("ix_book_files_book_id_format_id_historical_text")
                        .HasFilter("status_id != 6");

                    NpgsqlIndexBuilderExtensions.AreNullsDistinct(b.HasIndex("BookId", "FormatId", "HistoricalText"), false);

                    b.ToTable("book_files", null, t =>
                        {
                            t.HasCheckConstraint("CK_book_files_completed", "(completed_at is NULL AND status_id < 4) OR (completed_at is NOT NULL AND status_id >= 4)");

                            t.HasCheckConstraint("CK_book_files_deleted", "(deleted_at is NULL AND status_id != 7) OR (deleted_at is NOT NULL AND status_id = 7)");

                            t.HasCheckConstraint("CK_book_files_hidden", "(hidden_at is NULL AND status_id < 6) OR (hidden_at is NOT NULL AND status_id >=6)");

                            t.HasCheckConstraint("CK_book_files_original_size_positive", "original_size > 0");

                            t.HasCheckConstraint("CK_book_files_stored_size_not_negative", "stored_size>=0");

                            t.HasCheckConstraint("ck_book_files_archive_rule", "(status_id = 6 AND is_readable = true AND hidden_at IS NOT NULL) OR (status_id != 6 AND hidden_at IS NULL)");

                            t.HasCheckConstraint("ck_book_files_readable_format", "NOT (\"is_readable\" = true) OR (\"format_id\" = 1)");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFileStatus", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_book_file_statuses");

                    b.ToTable("book_file_statuses", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "В ожидании загрузки"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "Файл загружен, не обработан"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Обработка"
                        },
                        new
                        {
                            Id = 4L,
                            Name = "Готов"
                        },
                        new
                        {
                            Id = 5L,
                            Name = "Ошибка"
                        },
                        new
                        {
                            Id = 6L,
                            Name = "Архив"
                        },
                        new
                        {
                            Id = 7L,
                            Name = "Удален"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFragment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookFileId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_file_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("EndPos")
                        .HasColumnType("integer")
                        .HasColumnName("end_pos");

                    b.Property<int>("Position")
                        .HasColumnType("integer")
                        .HasColumnName("position");

                    b.Property<int>("StartPos")
                        .HasColumnType("integer")
                        .HasColumnName("start_pos");

                    b.Property<string>("StorageUrl")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("storage_url");

                    b.HasKey("Id")
                        .HasName("pk_book_fragments");

                    b.HasIndex("BookFileId", "Position")
                        .IsUnique()
                        .HasDatabaseName("ix_book_fragments_book_file_id_position");

                    b.ToTable("book_fragments", null, t =>
                        {
                            t.HasCheckConstraint("CK_book_fragments_position_positive", "position >= 0");

                            t.HasCheckConstraint("CK_book_fragments_start_end_pos_positive", "end_pos >= start_pos AND start_pos >= 0");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookParticipation", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<long>("PersonId")
                        .HasColumnType("bigint")
                        .HasColumnName("person_id");

                    b.Property<long>("PersonRoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("person_role_id");

                    b.HasKey("Id")
                        .HasName("pk_book_participations");

                    b.HasIndex("BookId")
                        .HasDatabaseName("ix_book_participations_book_id");

                    b.HasIndex("PersonId")
                        .HasDatabaseName("ix_book_participations_person_id");

                    b.HasIndex("PersonRoleId")
                        .HasDatabaseName("ix_book_participations_person_role_id");

                    b.ToTable("book_participations", (string)null);
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookShelf", b =>
                {
                    b.Property<long>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<long>("ShelfId")
                        .HasColumnType("bigint")
                        .HasColumnName("shelf_id");

                    b.Property<DateTime>("AddedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("added_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.HasKey("BookId", "ShelfId")
                        .HasName("pk_book_shelf");

                    b.HasIndex("ShelfId")
                        .HasDatabaseName("ix_book_shelf_shelf_id");

                    b.ToTable("book_shelf", (string)null);
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Bookmark", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookFileId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_file_id");

                    b.Property<string>("Context")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("context");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Note")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("note");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.Property<string>("Xpointer")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("xpointer");

                    b.HasKey("Id")
                        .HasName("pk_bookmarks");

                    b.HasIndex("BookFileId")
                        .HasDatabaseName("ix_bookmarks_book_file_id");

                    b.HasIndex("UserId", "BookFileId", "Xpointer")
                        .IsUnique()
                        .HasDatabaseName("uq_bookmark_user_book_position");

                    b.ToTable("bookmarks", (string)null);
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Comment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<long?>("ParentCommentId")
                        .HasColumnType("bigint")
                        .HasColumnName("parent_comment_id");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("text");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_comments");

                    b.HasIndex("ParentCommentId")
                        .HasDatabaseName("ix_comments_parent_comment_id");

                    b.HasIndex("UserId")
                        .HasDatabaseName("ix_comments_user_id");

                    b.HasIndex("BookId", "CreatedAt")
                        .HasDatabaseName("ix_comments_book_id_created_at");

                    b.ToTable("comments", (string)null);
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.CommentReactions", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("CommentId")
                        .HasColumnType("bigint")
                        .HasColumnName("comment_id");

                    b.Property<short>("ReactionType")
                        .HasColumnType("smallint")
                        .HasColumnName("reaction_type");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_comment_reactions");

                    b.HasIndex("CommentId")
                        .HasDatabaseName("ix_comment_reactions_comment_id");

                    b.HasIndex("UserId", "CommentId")
                        .IsUnique()
                        .HasDatabaseName("ix_comment_reactions_user_id_comment_id");

                    b.ToTable("comment_reactions", null, t =>
                        {
                            t.HasCheckConstraint("ck_comment_reactions_reaction_type", "reaction_type IN (1, -1, 0)");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Content", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("ContentTypeId")
                        .HasColumnType("bigint")
                        .HasColumnName("content_type_id");

                    b.Property<long>("CountryId")
                        .HasColumnType("bigint")
                        .HasColumnName("country_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("description");

                    b.Property<long>("LanguageId")
                        .HasColumnType("bigint")
                        .HasColumnName("language_id");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("title");

                    b.Property<int?>("YearFrom")
                        .HasColumnType("integer")
                        .HasColumnName("year_from");

                    b.Property<int?>("YearTo")
                        .HasColumnType("integer")
                        .HasColumnName("year_to");

                    b.HasKey("Id")
                        .HasName("pk_contents");

                    b.HasIndex("ContentTypeId")
                        .HasDatabaseName("ix_contents_content_type_id");

                    b.HasIndex("CountryId")
                        .HasDatabaseName("ix_contents_country_id");

                    b.HasIndex("LanguageId")
                        .HasDatabaseName("ix_contents_language_id");

                    b.ToTable("contents", null, t =>
                        {
                            t.HasCheckConstraint("CK_contents_description_min_length", "LENGTH(TRIM(description))>=120");

                            t.HasCheckConstraint("CK_contents_title_alnum", "title ~ '[[:alnum:]]'");

                            t.HasCheckConstraint("CK_contents_years", "(year_from is null AND year_to is null) OR (year_from >= -10000 AND year_from <= year_to AND year_to <= EXTRACT(YEAR FROM CURRENT_TIMESTAMP)+1)OR (year_from is null AND -10000 <= year_to AND year_to <= EXTRACT(YEAR FROM CURRENT_TIMESTAMP)+1)OR (year_to is null AND -10000 <= year_from AND year_from <= EXTRACT(YEAR FROM CURRENT_TIMESTAMP)+1)");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            ContentTypeId = 20L,
                            CountryId = 1L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Монография является первой в отечественной литературе попыткой проследить процесс становления японского буддизма...",
                            LanguageId = 2L,
                            Title = "Буддизм в Японии",
                            YearFrom = 1993,
                            YearTo = 1993
                        },
                        new
                        {
                            Id = 2L,
                            ContentTypeId = 19L,
                            CountryId = 5L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Это — второе крупное исследование Ф. Броделя. Первое — «Средиземное море и мир Средиземноморья в эпоху Филиппа II»...",
                            LanguageId = 2L,
                            Title = "Структуры повседневности: возможное и невозможное",
                            YearFrom = 1979,
                            YearTo = 1979
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ContentParticipation", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("ContentId")
                        .HasColumnType("bigint")
                        .HasColumnName("content_id");

                    b.Property<long>("PersonId")
                        .HasColumnType("bigint")
                        .HasColumnName("person_id");

                    b.Property<long>("PersonRoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("person_role_id");

                    b.HasKey("Id")
                        .HasName("pk_content_participations");

                    b.HasIndex("ContentId")
                        .HasDatabaseName("ix_content_participations_content_id");

                    b.HasIndex("PersonId")
                        .HasDatabaseName("ix_content_participations_person_id");

                    b.HasIndex("PersonRoleId")
                        .HasDatabaseName("ix_content_participations_person_role_id");

                    b.ToTable("content_participations", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            ContentId = 1L,
                            PersonId = 1L,
                            PersonRoleId = 1L
                        },
                        new
                        {
                            Id = 2L,
                            ContentId = 2L,
                            PersonId = 2L,
                            PersonRoleId = 1L
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ContentType", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.Property<ContentNature>("Nature")
                        .HasColumnType("content_nature_enum")
                        .HasColumnName("nature");

                    b.HasKey("Id")
                        .HasName("pk_content_type");

                    b.ToTable("content_type", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Дневник",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 2L,
                            Name = "Письмо",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Мемуары",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 4L,
                            Name = "Автобиография",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 5L,
                            Name = "Хроника",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 6L,
                            Name = "Летопись",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 7L,
                            Name = "Манифест",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 8L,
                            Name = "Речь",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 9L,
                            Name = "Указ",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 10L,
                            Name = "Рассказ",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 11L,
                            Name = "Роман",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 12L,
                            Name = "Философский трактат",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 13L,
                            Name = "Религиозный трактат",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 14L,
                            Name = "Политический трактат",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 15L,
                            Name = "Биография",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 16L,
                            Name = "Путевые заметки",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 17L,
                            Name = "Сборник",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 18L,
                            Name = "Учебник",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 19L,
                            Name = "Историческое исследование",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 20L,
                            Name = "Монография",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 21L,
                            Name = "Научная статья",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 22L,
                            Name = "Другое",
                            Nature = ContentNature.Unknown
                        },
                        new
                        {
                            Id = 23L,
                            Name = "Комментарий",
                            Nature = ContentNature.Work
                        },
                        new
                        {
                            Id = 24L,
                            Name = "Священный текст",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 25L,
                            Name = "Стенограмма",
                            Nature = ContentNature.Document
                        },
                        new
                        {
                            Id = 26L,
                            Name = "Публицистика",
                            Nature = ContentNature.Work
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Country", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_countries");

                    b.ToTable("countries", null, t =>
                        {
                            t.HasCheckConstraint("ck_countries_name_not_empty", "LENGTH(TRIM(name))>0");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Россия"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "СССР"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Российская империя"
                        },
                        new
                        {
                            Id = 4L,
                            Name = "США"
                        },
                        new
                        {
                            Id = 5L,
                            Name = "Франция"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Format", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_formats");

                    b.HasIndex("Name")
                        .IsUnique()
                        .HasDatabaseName("ix_formats_name");

                    b.ToTable("formats", null, t =>
                        {
                            t.HasCheckConstraint("ck_formats_name_clean", "name = LOWER(TRIM(name)) AND LENGTH(TRIM(name)) > 0");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "fb2"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "epub"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Language", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_languages");

                    b.ToTable("languages", null, t =>
                        {
                            t.HasCheckConstraint("ck_languages_name_not_empty", "LENGTH(TRIM(name))>0");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Английский"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "Русский"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Французский"
                        },
                        new
                        {
                            Id = 4L,
                            Name = "Немецкий"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ModerationTask", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long?>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<long?>("CommentId")
                        .HasColumnType("bigint")
                        .HasColumnName("comment_id");

                    b.Property<string>("CommentText")
                        .IsRequired()
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("comment_text");

                    b.Property<long>("ModeratedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("moderated_by");

                    b.Property<DateTime?>("ResolvedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("resolved_at");

                    b.Property<long?>("ReviewId")
                        .HasColumnType("bigint")
                        .HasColumnName("review_id");

                    b.Property<DateTime>("StartedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("started_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<long>("StatusId")
                        .HasColumnType("bigint")
                        .HasColumnName("status_id");

                    b.HasKey("Id")
                        .HasName("pk_moderation_tasks");

                    b.HasIndex("BookId")
                        .IsUnique()
                        .HasDatabaseName("ix_moderation_tasks_one_book_only")
                        .HasFilter("status_id = 2");

                    b.HasIndex("CommentId")
                        .IsUnique()
                        .HasDatabaseName("ix_moderation_tasks_one_comment_only")
                        .HasFilter("status_id = 2");

                    b.HasIndex("ModeratedBy")
                        .HasDatabaseName("ix_moderation_tasks_moderated_by");

                    b.HasIndex("ReviewId")
                        .IsUnique()
                        .HasDatabaseName("ix_moderation_tasks_one_review_only")
                        .HasFilter("status_id = 2");

                    b.HasIndex("StatusId")
                        .HasDatabaseName("ix_moderation_tasks_status_id");

                    b.ToTable("moderation_tasks", null, t =>
                        {
                            t.HasCheckConstraint("ck_moderation_task_target", "((book_id IS NOT NULL)::int + (review_id IS NOT NULL)::int + (comment_id IS NOT NULL)::int) = 1");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Person", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("description");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_persons");

                    b.ToTable("persons", null, t =>
                        {
                            t.HasCheckConstraint("ck_persons_name_not_empty", "LENGTH(TRIM(name))>0");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Советский и российский востоковед-японист, литературовед, переводчица...",
                            Name = "Татьяна Петровна Григорьева"
                        },
                        new
                        {
                            Id = 2L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Французский историк, член Французской академии...",
                            Name = "Фернан Поль Ахилл Бродель"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.PersonRole", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<PersonRoleKind>("Kind")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("person_role_kind")
                        .HasDefaultValue(PersonRoleKind.Both)
                        .HasColumnName("kind");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_person_roles");

                    b.ToTable("person_roles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Kind = PersonRoleKind.Content,
                            Name = "Автор"
                        },
                        new
                        {
                            Id = 2L,
                            Kind = PersonRoleKind.Book,
                            Name = "Переводчик"
                        },
                        new
                        {
                            Id = 3L,
                            Kind = PersonRoleKind.Book,
                            Name = "Редактор"
                        },
                        new
                        {
                            Id = 4L,
                            Kind = PersonRoleKind.Both,
                            Name = "Иллюстратор"
                        },
                        new
                        {
                            Id = 5L,
                            Kind = PersonRoleKind.Book,
                            Name = "Составитель"
                        },
                        new
                        {
                            Id = 6L,
                            Kind = PersonRoleKind.Book,
                            Name = "Корректор (редактор подготовки текста)"
                        },
                        new
                        {
                            Id = 7L,
                            Kind = PersonRoleKind.Book,
                            Name = "Научный редактор"
                        },
                        new
                        {
                            Id = 8L,
                            Kind = PersonRoleKind.Book,
                            Name = "Редактор перевода"
                        },
                        new
                        {
                            Id = 9L,
                            Kind = PersonRoleKind.Content,
                            Name = "Адресат"
                        },
                        new
                        {
                            Id = 10L,
                            Kind = PersonRoleKind.Both,
                            Name = "Комментатор"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Publisher", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("description");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_publishers");

                    b.ToTable("publishers", null, t =>
                        {
                            t.HasCheckConstraint("ck_publishers_name_not_empty", "LENGTH(TRIM(name))>0");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            CreatedAt = new DateTime(2026, 4, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "«Прогре́сс» — советское государственное и российское частное издательство. Основано в 1964 году на базе гуманитарных редакций «Издательства иностранной литературы» и «Издательства литературы на иностранных языках». Специализировалось на выпуске переводной гуманитарной и художественной литературы.",
                            Name = "Прогресс"
                        },
                        new
                        {
                            Id = 2L,
                            CreatedAt = new DateTime(2026, 4, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "«Восто́чная литерату́ра» РАН — издательская фирма, специализирующаяся на различных отраслях востоковедения и африканистики.",
                            Name = "Восточная литература"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReadingProgress", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookFileId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_file_id");

                    b.Property<int>("ParaIndex")
                        .HasColumnType("integer")
                        .HasColumnName("para_index");

                    b.Property<decimal>("Percentage")
                        .HasColumnType("numeric")
                        .HasColumnName("percentage");

                    b.Property<DateTime>("ReadingDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reading_date");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_reading_progresses");

                    b.HasIndex("BookFileId")
                        .HasDatabaseName("ix_reading_progresses_book_file_id");

                    b.HasIndex("UserId", "BookFileId")
                        .IsUnique()
                        .HasDatabaseName("ix_reading_progresses_user_id_book_file_id");

                    b.ToTable("reading_progresses", (string)null);
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Report", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long?>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<long?>("CommentId")
                        .HasColumnType("bigint")
                        .HasColumnName("comment_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("description");

                    b.Property<long?>("ModerationTaskId")
                        .HasColumnType("bigint")
                        .HasColumnName("moderation_task_id");

                    b.Property<long>("ReasonTypeId")
                        .HasColumnType("bigint")
                        .HasColumnName("reason_type_id");

                    b.Property<long?>("ReviewId")
                        .HasColumnType("bigint")
                        .HasColumnName("review_id");

                    b.HasKey("Id")
                        .HasName("pk_reports");

                    b.HasIndex("BookId")
                        .HasDatabaseName("ix_reports_book_id");

                    b.HasIndex("CommentId")
                        .HasDatabaseName("ix_reports_comment_id");

                    b.HasIndex("CreatedBy")
                        .HasDatabaseName("ix_reports_created_by");

                    b.HasIndex("ModerationTaskId")
                        .HasDatabaseName("ix_reports_moderation_task_id");

                    b.HasIndex("ReasonTypeId")
                        .HasDatabaseName("ix_reports_reason_type_id");

                    b.HasIndex("ReviewId")
                        .HasDatabaseName("ix_reports_review_id");

                    b.ToTable("reports", null, t =>
                        {
                            t.HasCheckConstraint("ck_report_target", "((book_id IS NOT NULL)::int + (review_id IS NOT NULL)::int + (comment_id IS NOT NULL)::int) = 1");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReportReasonType", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_report_reasons");

                    b.ToTable("report_reasons", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Спам"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "Ненормативная лексика"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Нарушение авторских прав"
                        },
                        new
                        {
                            Id = 4L,
                            Name = "Терроризм и экстремизм"
                        },
                        new
                        {
                            Id = 5L,
                            Name = "Иное"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReportStatus", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_report_statuses");

                    b.ToTable("report_statuses", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Новое"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "В работе"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Принято"
                        },
                        new
                        {
                            Id = 4L,
                            Name = "Отклонено"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Review", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("BookId")
                        .HasColumnType("bigint")
                        .HasColumnName("book_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<string>("ReviewText")
                        .HasMaxLength(5000)
                        .HasColumnType("character varying(5000)")
                        .HasColumnName("review_text");

                    b.Property<short>("Score")
                        .HasColumnType("smallint")
                        .HasColumnName("score");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_reviews");

                    b.HasIndex("BookId")
                        .HasDatabaseName("ix_reviews_book_id");

                    b.HasIndex("UserId", "BookId")
                        .IsUnique()
                        .HasDatabaseName("ix_reviews_user_id_book_id")
                        .HasFilter("is_deleted = false");

                    b.ToTable("reviews", null, t =>
                        {
                            t.HasCheckConstraint("ck_review_rating", "score IN (1,2,3,4,5)");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReviewReactions", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<short>("ReactionType")
                        .HasColumnType("smallint")
                        .HasColumnName("reaction_type");

                    b.Property<long>("ReviewId")
                        .HasColumnType("bigint")
                        .HasColumnName("review_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_review_reactions");

                    b.HasIndex("ReviewId")
                        .HasDatabaseName("ix_review_reactions_review_id");

                    b.HasIndex("UserId", "ReviewId")
                        .IsUnique()
                        .HasDatabaseName("ix_review_reactions_user_id_review_id");

                    b.ToTable("review_reactions", null, t =>
                        {
                            t.HasCheckConstraint("ck_review_reactions_reaction_type", "reaction_type IN (1, -1, 0)");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Selection", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("description");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("name");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id")
                        .HasName("pk_selections");

                    b.HasIndex("CreatedBy")
                        .HasDatabaseName("ix_selections_created_by");

                    b.ToTable("selections", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            CreatedBy = 1L,
                            Description = "",
                            IsActive = true,
                            Name = "Экономическая история"
                        },
                        new
                        {
                            Id = 2L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            CreatedBy = 1L,
                            Description = "",
                            IsActive = true,
                            Name = "История культуры"
                        },
                        new
                        {
                            Id = 3L,
                            CreatedAt = new DateTime(2025, 11, 20, 0, 0, 0, 0, DateTimeKind.Utc),
                            CreatedBy = 1L,
                            Description = "",
                            IsActive = true,
                            Name = "История мира"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Shelf", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("name");

                    b.Property<long>("ShelfTypeId")
                        .HasColumnType("bigint")
                        .HasColumnName("shelf_type_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_shelves");

                    b.HasIndex("ShelfTypeId")
                        .HasDatabaseName("ix_shelves_shelf_type_id");

                    b.HasIndex("UserId", "ShelfTypeId")
                        .HasDatabaseName("IX_Shelves_UserId_ShelfTypeId");

                    b.ToTable("shelves", null, t =>
                        {
                            t.HasCheckConstraint("ck_shelves_name_not_empty", "LENGTH(TRIM(name))>0");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ShelfType", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("code");

                    b.HasKey("Id")
                        .HasName("pk_shelf_types");

                    b.ToTable("shelf_types", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Code = "FAVORITES"
                        },
                        new
                        {
                            Id = 2L,
                            Code = "READ"
                        },
                        new
                        {
                            Id = 3L,
                            Code = "CUSTOM"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Tag", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("name");

                    b.Property<long?>("ParentTagId")
                        .HasColumnType("bigint")
                        .HasColumnName("parent_tag_id");

                    b.Property<long?>("RelationTypeId")
                        .HasColumnType("bigint")
                        .HasColumnName("relation_type_id");

                    b.Property<long>("TagTypeId")
                        .HasColumnType("bigint")
                        .HasColumnName("tag_type_id");

                    b.HasKey("Id")
                        .HasName("pk_tags");

                    b.HasIndex("ParentTagId")
                        .HasDatabaseName("ix_tags_parent_tag_id");

                    b.HasIndex("RelationTypeId")
                        .HasDatabaseName("ix_tags_relation_type_id");

                    b.HasIndex("TagTypeId")
                        .HasDatabaseName("ix_tags_tag_type_id");

                    b.ToTable("tags", null, t =>
                        {
                            t.HasCheckConstraint("ck_tags_name_not_empty", "LENGTH(TRIM(name))>0");
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.TagRelationType", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Description")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("description");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_tag_relation_type");

                    b.HasIndex("Name")
                        .IsUnique()
                        .HasDatabaseName("ix_tag_relation_type_name");

                    b.ToTable("tag_relation_type", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Description = "Синонимия (теги обозначают одно и то же понятие)",
                            Name = "Синоним"
                        },
                        new
                        {
                            Id = 2L,
                            Description = "Часть/целое",
                            Name = "Часть"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.TagType", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_tag_types");

                    b.ToTable("tag_types", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Время"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "Место"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Социум"
                        });
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Theme", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("name");

                    b.Property<long?>("ParentThemeId")
                        .HasColumnType("bigint")
                        .HasColumnName("parent_theme_id");

                    b.HasKey("Id")
                        .HasName("pk_themes");

                    b.HasIndex("ParentThemeId")
                        .HasDatabaseName("ix_themes_parent_theme_id");

                    b.ToTable("themes", null, t =>
                        {
                            t.HasCheckConstraint("ck_themes_name_not_empty", "LENGTH(TRIM(name))>0");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "Политическая история"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "Военная история"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "Философия"
                        },
                        new
                        {
                            Id = 6L,
                            Name = "Искусство"
                        },
                        new
                        {
                            Id = 7L,
                            Name = "Религия"
                        },
                        new
                        {
                            Id = 8L,
                            Name = "Всеобщая и мировая история",
                            ParentThemeId = 1L
                        },
                        new
                        {
                            Id = 9L,
                            Name = "Региональная и национальная история",
                            ParentThemeId = 1L
                        },
                        new
                        {
                            Id = 10L,
                            Name = "История Европы",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 11L,
                            Name = "История России",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 12L,
                            Name = "История Азии",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 5L,
                            Name = "История Японии",
                            ParentThemeId = 12L
                        },
                        new
                        {
                            Id = 4L,
                            Name = "История Китая",
                            ParentThemeId = 12L
                        },
                        new
                        {
                            Id = 13L,
                            Name = "История Африки",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 14L,
                            Name = "История Америки",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 15L,
                            Name = "История Австралазии и Тихоокеании",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 16L,
                            Name = "История других земель",
                            ParentThemeId = 9L
                        },
                        new
                        {
                            Id = 17L,
                            Name = "История: отдельные темы и события",
                            ParentThemeId = 1L
                        },
                        new
                        {
                            Id = 18L,
                            Name = "Западная философия",
                            ParentThemeId = 3L
                        },
                        new
                        {
                            Id = 19L,
                            Name = "Античная философия",
                            ParentThemeId = 18L
                        },
                        new
                        {
                            Id = 20L,
                            Name = "Философия Средних веков и эпохи Ренессанса",
                            ParentThemeId = 18L
                        },
                        new
                        {
                            Id = 21L,
                            Name = "Философия эпохи Нового времени",
                            ParentThemeId = 18L
                        },
                        new
                        {
                            Id = 22L,
                            Name = "Современная философия",
                            ParentThemeId = 18L
                        },
                        new
                        {
                            Id = 23L,
                            Name = "Восточная философия",
                            ParentThemeId = 3L
                        },
                        new
                        {
                            Id = 24L,
                            Name = "Исламская и арабская философия",
                            ParentThemeId = 3L
                        },
                        new
                        {
                            Id = 25L,
                            Name = "Социальная и политическая философия",
                            ParentThemeId = 3L
                        },
                        new
                        {
                            Id = 26L,
                            Name = "Русская философия",
                            ParentThemeId = 3L
                        });
                });

            modelBuilder.Entity("Chronolibris.Infrastructure.Data.User", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("integer")
                        .HasColumnName("access_failed_count");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .IsRequired()
                        .HasMaxLength(36)
                        .IsUnicode(false)
                        .HasColumnType("character(36)")
                        .HasColumnName("concurrency_stamp")
                        .IsFixedLength();

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("deleted_at");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("email");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("boolean")
                        .HasColumnName("email_confirmed");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("first_name");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_deleted");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("last_name");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("lockout_enabled");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("lockout_end");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("normalized_email");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("normalized_user_name");

                    b.Property<string>("PasswordHash")
                        .HasMaxLength(256)
                        .IsUnicode(false)
                        .HasColumnType("character(256)")
                        .HasColumnName("password_hash")
                        .IsFixedLength();

                    b.Property<string>("PhoneNumber")
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("phone_number")
                        .IsFixedLength(false);

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("boolean")
                        .HasColumnName("phone_number_confirmed");

                    b.Property<DateTime>("RegisteredAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("registered_at")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("SecurityStamp")
                        .IsRequired()
                        .HasMaxLength(36)
                        .IsUnicode(false)
                        .HasColumnType("character varying(36)")
                        .HasColumnName("security_stamp")
                        .IsFixedLength(false);

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("two_factor_enabled");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("user_name");

                    b.HasKey("Id")
                        .HasName("pk_users");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("users", null, t =>
                        {
                            t.HasCheckConstraint("ck_users_first_name_correct_length", "LENGTH(TRIM(first_name))>0");

                            t.HasCheckConstraint("ck_users_last_name_correct_length", "LENGTH(TRIM(last_name))>0");

                            t.HasCheckConstraint("ck_users_user_name_correct_length", "LENGTH(TRIM(user_name))>4");
                        });
                });

            modelBuilder.Entity("ContentTheme", b =>
                {
                    b.Property<long>("content_id")
                        .HasColumnType("bigint")
                        .HasColumnName("content_id");

                    b.Property<long>("theme_id")
                        .HasColumnType("bigint")
                        .HasColumnName("theme_id");

                    b.HasKey("content_id", "theme_id")
                        .HasName("pk_content_theme");

                    b.HasIndex("theme_id")
                        .HasDatabaseName("ix_content_theme_theme_id");

                    b.ToTable("content_theme", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole<long>", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasMaxLength(36)
                        .IsUnicode(false)
                        .HasColumnType("character(36)")
                        .HasColumnName("concurrency_stamp")
                        .IsFixedLength();

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("name");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("normalized_name");

                    b.HasKey("Id")
                        .HasName("pk_roles");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex");

                    b.ToTable("roles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "admin",
                            NormalizedName = "ADMIN"
                        },
                        new
                        {
                            Id = 2L,
                            Name = "reader",
                            NormalizedName = "READER"
                        },
                        new
                        {
                            Id = 3L,
                            Name = "moderator",
                            NormalizedName = "MODERATOR"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<long>", b =>
                {
                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.Property<long>("RoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("role_id");

                    b.HasKey("UserId", "RoleId")
                        .HasName("pk_user_role");

                    b.HasIndex("RoleId")
                        .HasDatabaseName("ix_user_role_role_id");

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasDatabaseName("ix_user_role_user_id");

                    b.ToTable("user_role", (string)null);
                });

            modelBuilder.Entity("content_tags", b =>
                {
                    b.Property<long>("contents_id")
                        .HasColumnType("bigint")
                        .HasColumnName("contents_id");

                    b.Property<long>("tags_id")
                        .HasColumnType("bigint")
                        .HasColumnName("tags_id");

                    b.HasKey("contents_id", "tags_id")
                        .HasName("pk_content_tags");

                    b.HasIndex("tags_id")
                        .HasDatabaseName("ix_content_tags_tags_id");

                    b.ToTable("content_tags", (string)null);
                });

            modelBuilder.Entity("BookSelection", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", null)
                        .WithMany()
                        .HasForeignKey("book_id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_selection_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Selection", null)
                        .WithMany()
                        .HasForeignKey("selection_id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_selection_selections_selection_id");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Book", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Country", "Country")
                        .WithMany("Books")
                        .HasForeignKey("CountryId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_books_countries_country_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("CreatedBy")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_books_users_created_by");

                    b.HasOne("Chronolibris.Domain.Entities.Language", "Language")
                        .WithMany("Books")
                        .HasForeignKey("LanguageId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_books_languages_language_id");

                    b.HasOne("Chronolibris.Domain.Entities.Publisher", "Publisher")
                        .WithMany("Books")
                        .HasForeignKey("PublisherId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_books_publishers_publisher_id");

                    b.Navigation("Country");

                    b.Navigation("Language");

                    b.Navigation("Publisher");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookContent", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany("BookContents")
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_book_content_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Content", "Content")
                        .WithMany("BookContents")
                        .HasForeignKey("ContentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_book_content_contents_content_id");

                    b.Navigation("Book");

                    b.Navigation("Content");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFile", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany()
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_files_books_book_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("CreatedBy")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_files_users_created_by");

                    b.HasOne("Chronolibris.Domain.Entities.Format", "Format")
                        .WithMany()
                        .HasForeignKey("FormatId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_book_files_formats_format_id");

                    b.HasOne("Chronolibris.Domain.Entities.BookFileStatus", "BookFileStatus")
                        .WithMany("BookFiles")
                        .HasForeignKey("StatusId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_book_files_book_file_statuses_status_id");

                    b.Navigation("Book");

                    b.Navigation("BookFileStatus");

                    b.Navigation("Format");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFragment", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.BookFile", "BookFile")
                        .WithMany("Fragments")
                        .HasForeignKey("BookFileId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_fragments_book_files_book_file_id");

                    b.Navigation("BookFile");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookParticipation", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany("Participations")
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_participations_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Person", "Person")
                        .WithMany("BookParticipations")
                        .HasForeignKey("PersonId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_book_participations_persons_person_id");

                    b.HasOne("Chronolibris.Domain.Entities.PersonRole", "PersonRole")
                        .WithMany()
                        .HasForeignKey("PersonRoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_participations_person_roles_person_role_id");

                    b.Navigation("Book");

                    b.Navigation("Person");

                    b.Navigation("PersonRole");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookShelf", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany("BookShelves")
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_shelf_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Shelf", "Shelf")
                        .WithMany("BookShelves")
                        .HasForeignKey("ShelfId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_book_shelf_shelves_shelf_id");

                    b.Navigation("Book");

                    b.Navigation("Shelf");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Bookmark", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.BookFile", "BookFile")
                        .WithMany("Bookmarks")
                        .HasForeignKey("BookFileId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_bookmarks_book_files_book_file_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_bookmarks_users_user_id");

                    b.Navigation("BookFile");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Comment", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany("Comments")
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_comments_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Comment", "ParentComment")
                        .WithMany("Replies")
                        .HasForeignKey("ParentCommentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_comments_comments_parent_comment_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_comments_users_user_id");

                    b.Navigation("Book");

                    b.Navigation("ParentComment");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.CommentReactions", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Comment", null)
                        .WithMany("CommentReactions")
                        .HasForeignKey("CommentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_comment_reactions_comments_comment_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_comment_reactions_users_user_id");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Content", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.ContentType", "ContentType")
                        .WithMany("Contents")
                        .HasForeignKey("ContentTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_contents_content_type_content_type_id");

                    b.HasOne("Chronolibris.Domain.Entities.Country", "Country")
                        .WithMany("Contents")
                        .HasForeignKey("CountryId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_contents_countries_country_id");

                    b.HasOne("Chronolibris.Domain.Entities.Language", "Language")
                        .WithMany("Contents")
                        .HasForeignKey("LanguageId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_contents_languages_language_id");

                    b.Navigation("ContentType");

                    b.Navigation("Country");

                    b.Navigation("Language");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ContentParticipation", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Content", "Content")
                        .WithMany("Participations")
                        .HasForeignKey("ContentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_content_participations_contents_content_id");

                    b.HasOne("Chronolibris.Domain.Entities.Person", "Person")
                        .WithMany("ContentParticipations")
                        .HasForeignKey("PersonId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_content_participations_persons_person_id");

                    b.HasOne("Chronolibris.Domain.Entities.PersonRole", "PersonRole")
                        .WithMany()
                        .HasForeignKey("PersonRoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_content_participations_person_roles_person_role_id");

                    b.Navigation("Content");

                    b.Navigation("Person");

                    b.Navigation("PersonRole");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ModerationTask", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany()
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_moderation_tasks_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Comment", "Comment")
                        .WithMany()
                        .HasForeignKey("CommentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_moderation_tasks_comments_comment_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("ModeratedBy")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_moderation_tasks_users_moderated_by");

                    b.HasOne("Chronolibris.Domain.Entities.Review", "Review")
                        .WithMany()
                        .HasForeignKey("ReviewId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_moderation_tasks_reviews_review_id");

                    b.HasOne("Chronolibris.Domain.Entities.ReportStatus", "Status")
                        .WithMany("Tasks")
                        .HasForeignKey("StatusId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_moderation_tasks_report_statuses_status_id");

                    b.Navigation("Book");

                    b.Navigation("Comment");

                    b.Navigation("Review");

                    b.Navigation("Status");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReadingProgress", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.BookFile", "BookFile")
                        .WithMany("Readings")
                        .HasForeignKey("BookFileId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_reading_progresses_book_files_book_file_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_reading_progresses_users_user_id");

                    b.Navigation("BookFile");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Report", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", "Book")
                        .WithMany()
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_reports_books_book_id");

                    b.HasOne("Chronolibris.Domain.Entities.Comment", "Comment")
                        .WithMany()
                        .HasForeignKey("CommentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_reports_comments_comment_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("CreatedBy")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_reports_users_created_by");

                    b.HasOne("Chronolibris.Domain.Entities.ModerationTask", "ModerationTask")
                        .WithMany("Reports")
                        .HasForeignKey("ModerationTaskId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_reports_moderation_tasks_moderation_task_id");

                    b.HasOne("Chronolibris.Domain.Entities.ReportReasonType", "ReasonType")
                        .WithMany("Reports")
                        .HasForeignKey("ReasonTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_reports_report_reasons_reason_type_id");

                    b.HasOne("Chronolibris.Domain.Entities.Review", "Review")
                        .WithMany()
                        .HasForeignKey("ReviewId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_reports_reviews_review_id");

                    b.Navigation("Book");

                    b.Navigation("Comment");

                    b.Navigation("ModerationTask");

                    b.Navigation("ReasonType");

                    b.Navigation("Review");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Review", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Book", null)
                        .WithMany("Reviews")
                        .HasForeignKey("BookId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_reviews_books_book_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_reviews_users_user_id");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReviewReactions", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Review", null)
                        .WithMany("ReviewsRatings")
                        .HasForeignKey("ReviewId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_review_reactions_reviews_review_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_review_reactions_users_user_id");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Selection", b =>
                {
                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("CreatedBy")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_selections_users_created_by");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Shelf", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.ShelfType", "ShelfType")
                        .WithMany()
                        .HasForeignKey("ShelfTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_shelves_shelf_types_shelf_type_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_shelves_users_user_id");

                    b.Navigation("ShelfType");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Tag", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Tag", "ParentTag")
                        .WithMany("ChildTags")
                        .HasForeignKey("ParentTagId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_tags_tags_parent_tag_id");

                    b.HasOne("Chronolibris.Domain.Entities.TagRelationType", "RelationType")
                        .WithMany("Tags")
                        .HasForeignKey("RelationTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_tags_tag_relation_type_relation_type_id");

                    b.HasOne("Chronolibris.Domain.Entities.TagType", "TagType")
                        .WithMany("Tags")
                        .HasForeignKey("TagTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_tags_tag_types_tag_type_id");

                    b.Navigation("ParentTag");

                    b.Navigation("RelationType");

                    b.Navigation("TagType");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Theme", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Theme", "ParentTheme")
                        .WithMany("SubThemes")
                        .HasForeignKey("ParentThemeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_themes_themes_parent_theme_id");

                    b.Navigation("ParentTheme");
                });

            modelBuilder.Entity("ContentTheme", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Content", null)
                        .WithMany()
                        .HasForeignKey("content_id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_content_theme_contents_content_id");

                    b.HasOne("Chronolibris.Domain.Entities.Theme", null)
                        .WithMany()
                        .HasForeignKey("theme_id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_content_theme_themes_theme_id");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<long>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole<long>", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_user_role_roles_role_id");

                    b.HasOne("Chronolibris.Infrastructure.Data.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_user_role_users_user_id");
                });

            modelBuilder.Entity("content_tags", b =>
                {
                    b.HasOne("Chronolibris.Domain.Entities.Content", null)
                        .WithMany()
                        .HasForeignKey("contents_id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_content_tags_contents_contents_id");

                    b.HasOne("Chronolibris.Domain.Entities.Tag", null)
                        .WithMany()
                        .HasForeignKey("tags_id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_content_tags_tags_tags_id");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Book", b =>
                {
                    b.Navigation("BookContents");

                    b.Navigation("BookShelves");

                    b.Navigation("Comments");

                    b.Navigation("Participations");

                    b.Navigation("Reviews");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFile", b =>
                {
                    b.Navigation("Bookmarks");

                    b.Navigation("Fragments");

                    b.Navigation("Readings");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.BookFileStatus", b =>
                {
                    b.Navigation("BookFiles");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Comment", b =>
                {
                    b.Navigation("CommentReactions");

                    b.Navigation("Replies");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Content", b =>
                {
                    b.Navigation("BookContents");

                    b.Navigation("Participations");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ContentType", b =>
                {
                    b.Navigation("Contents");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Country", b =>
                {
                    b.Navigation("Books");

                    b.Navigation("Contents");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Language", b =>
                {
                    b.Navigation("Books");

                    b.Navigation("Contents");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ModerationTask", b =>
                {
                    b.Navigation("Reports");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Person", b =>
                {
                    b.Navigation("BookParticipations");

                    b.Navigation("ContentParticipations");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Publisher", b =>
                {
                    b.Navigation("Books");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReportReasonType", b =>
                {
                    b.Navigation("Reports");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.ReportStatus", b =>
                {
                    b.Navigation("Tasks");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Review", b =>
                {
                    b.Navigation("ReviewsRatings");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Shelf", b =>
                {
                    b.Navigation("BookShelves");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Tag", b =>
                {
                    b.Navigation("ChildTags");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.TagRelationType", b =>
                {
                    b.Navigation("Tags");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.TagType", b =>
                {
                    b.Navigation("Tags");
                });

            modelBuilder.Entity("Chronolibris.Domain.Entities.Theme", b =>
                {
                    b.Navigation("SubThemes");
                });
#pragma warning restore 612, 618
        }
    }
}
